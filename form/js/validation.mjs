class Validation {
    constructor(selector, arr){
        ...
        arr.forEach(opt=>{
            this.btnSubmit.addEventListener('click', e=>{
                if(opt.type==='text') if(!this.isTxt(opt.name, opt.len)) e.preventDefault();
                if(opt.type==='email') if(!this.isEmail(opt.name, opt.len)) e.preventDefault();
                if(opt.type==='check') if(!this.isCheck(opt.name)) e.preventDefault();
                if(opt.type==='select') if(!this.isSelect(opt.name)) e.preventDefault();
                if(opt.type==='password') if(!this.isPwd(opt.name[0], opt.name[1], opt.len)) e.preventDefault();
            })
        })
    }
    ...
    isTxt(name, len){
        ...
        if(txt.length > len){
            ...
            errMsg.append(`텍스트를 ${len+1}글자 이상 입력하세요`);
    ...
    isEmail(name, len){
        ...
        if(txt.length > len && /@/.test(txt)){
            ...
            const errMsg = document.createElement('p');
            errMsg.append(`@를 포함한 메일 주소를 ${len}글자 이상 입력하세요.`);
            input.closest('td').append(errMsg);
